...
# Onboard Scripts
locals {
  user_data = templatefile("${path.module}/templates/backend/cloud-config.yml", {
    index_html      = replace(file("${path.module}/templates/backend/index.html"), "/[\\n\\r]/", "")
    f5_logo_rgb_svg = base64gzip(file("${path.module}/templates/backend/f5-logo-rgb.svg"))
    styles_css      = base64gzip(file("${path.module}/templates/backend/styles.css"))
  })
}
